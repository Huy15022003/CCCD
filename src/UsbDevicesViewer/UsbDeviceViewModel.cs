    ...
    public class UsbDeviceViewModel : ViewModelBase
    {
        ...
        public void Refresh(UsbDevice usbDevice)
        {
            ...
            foreach (UsbDeviceProperty usbDeviceProperty in usbDevice.Properties)
            {
                ...
                if (usbDeviceProperty.HasSameKey(UsbDeviceWinApi.DevicePropertyKeys.DEVPKEY_Device_Manufacturer))
                {
                    this.Manufacturer = values[0];
                    this.OnPropertyChanged(() => this.Manufacturer);
                }
            ...
            if (string.IsNullOrEmpty(this.SerialNumber))
            {
                ...
                foreach (UsbDeviceRegistryProperty usbDeviceRegistryProperty in usbDevice.RegistryProperties)
                {
                    ...
                    string regPropDescription = usbDeviceRegistryProperty.GetDescription().ToLower();
                    if (string.IsNullOrEmpty(this.Manufacturer) && regPropDescription.IndexOf("manufacturer", StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        this.Manufacturer = values[0];
                        this.OnPropertyChanged(() => this.Manufacturer);
                    }
                    else if (regPropDescription.IndexOf("serial", StringComparison.OrdinalIgnoreCase) >= 0)
                    {
                        this.SerialNumber = values[0];
            ...
            if (string.IsNullOrEmpty(this.SerialNumber) && !string.IsNullOrEmpty(this.DeviceId))
            {
                var parts = this.DeviceId.Split('\\');
                if (parts.Length > 2)
                {
                    this.SerialNumber = parts[2];
                    this.OnPropertyChanged(() => this.SerialNumber);
                }
            }

            this.Manufacturer = this.Manufacturer ?? "Không xác định";
            this.SerialNumber = this.SerialNumber ?? "Không có";
